...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/viauco/pgsql) and is the recommended method of installation.


```bash
docker pull viauco/pgsql
```

Alternatively you can build the image yourself.

```bash
docker build -t viauco/pgsql github.com/viauco/docker-postgresql
```
...
```bash
docker run --name postgresql -itd --restart always \
  viauco/pgsql -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  viauco/pgsql -c logging_collector=on
```
